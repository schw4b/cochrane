...
parse.studies = function(file, path) {
  ...
  if (l > 0) { # in case there is outcome data in the file
    ...
    message(paste("No outcome data found in:", file))
    return(NA)
  }
}
...
parse.ma = function(file, path) {
  ...
  if (l > 0) { # in case there is outcome data in the file
    ...
    # message(paste("No outcome data found for", file)) # message appears twice
    return(NA)
  }
}
...
get.review = function(doi, path, show.terms = TRUE) {
  ...
  if (!session_with_ua$response$status_code == 200) {
    ...
    if (session_data$response$status_code == 200) {
      ...
      message(paste(Sys.time(), "download successful for", id))
